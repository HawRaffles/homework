...
$soloLogger = SingletonLogger::getInstance(new Logger('general'));
$soloLogger->pushHandler(new StreamHandler($config['logfile']['error'], level::Error))
    ->pushHandler(new StreamHandler($config['logfile']['warning'], level::Warning))
...
try {
    $urlData = new ConvertUrl($config['datafile'], $carbon->addYear());
    switch ($inputType) {
        case 0:
            exit();
        case 1:
            do {
                $validUrl = new CheckUrl(new Client(), $config['timeout'], $config['responses'], $config['ua']);
